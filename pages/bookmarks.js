...
const makeBookmarkListItem = (bookmarks, menu, removalAction, reverseRemoval) => ({ index, style }) =>
	<div style={style} className={bookmarks[menu][index].markedForRemoval ? 'bookmark-item bookmark-removed' : 'bookmark-item'}>
		<div className="bookmark-item-inner">
			<div className="item-contents">
				...
				/>
				<h1>{ menu === 'comics' ? bookmarks[menu][index].title : bookmarks[menu][index].name}</h1>
			</div>
			<div className="bookmark-actions">
				{
					bookmarks[menu][index].markedForRemoval
						? <button type="button" className="item-undo" onClick={() => reverseRemoval(bookmarks[menu][index].id)}>Undo</button>
						: <button type="button" className="item-remove" onClick={() => removalAction(bookmarks[menu][index].id)}>Remove</button>
				}
			</div>
		</div>
	</div>
				...
				<div className="bookmarks-container">
					{
						activeBookmarks
							? <List
								...
								width="100%"
							>
								{ makeBookmarkListItem(activeBookmarks, activeMenu, markForRemoval, reverRemoval) }
							</List>
						: null
					}
				</div>
